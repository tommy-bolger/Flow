    ...
    */
    ...
    private function setPageFilter($filter_columns) {
        ...
        if(!empty($filter_columns)) {
            ...
            $this->record_filter = array_merge($this->record_filter, $page_filter);
        }
    }
    ...
    private function addFilterOptions($filter_field_name, $filter_options) {
        ...
        if(!empty($filter_options)) {            
            $this->record_filter_form = new Form("{$this->name}_filter_form", Http::getPageUrl());
                ...
                case 'delete':
                    ...
                    if(!empty($delete_sort_order)) {
                        ...
                        db()->query("
                            ...
                            {$sort_order_where_clause}
                        ", $this->record_filter + array($delete_sort_order));
                    }
                    else {
        ...
        if(!empty($current_sort_order) && is_numeric($current_sort_order)) {
            ...
            if(!empty($target_sort_order)) {            
                ...
                $affected_count = db()->update($this->edit_table_name, array(
                    $this->table_sort_field => $current_sort_order
                ), (array($this->table_sort_field => $target_sort_order) + $this->record_filter));
    ...
    private function generateTableLink($action = '', $id = '') {
        ...
        switch($action) {
            ...
            case 'add':
                ...
                $page_table_parameters = Http::getPageBaseUrl() . $this->edit_page;
                break;
            default:
        ...
        if(isset($this->page_filter)) {
            foreach($this->page_filter as $column_name => $column_value) {
                $page_filter_parameters .= "&{$column_name}={$column_value}";
            }
        }
	...
	public function toHtml() {
        ...
        if(isset($this->record_filter_form)) {
            ...
            $upper_table_name = strtoupper($this->name);
            
            $edit_table_html .= str_replace(array_keys($form_template), $form_template, "
                <span style=\"float: right;\">
                    {{{$upper_table_name}_FILTER_FORM}}
                        {{{$upper_table_name}_FILTER_DROPDOWN_LABEL}} {{{$upper_table_name}_FILTER_DROPDOWN}}
                        {{{$upper_table_name}_FILTER_SUBMIT}}
                    </form>
                </span>
            ");
        }
