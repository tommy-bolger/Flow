...
class Database {
    ...
    */
    ...
    public static function getDatabase($database_connection = NULL) {            
        ...
        if(!isset(self::$database_connections[$database_connection])) {
            ...
            if($database_connection == 'default') {
                $engine = config('framework')->getParameter("database_engine");
     ...
     * @return void
     */
    public function connect($engine, $host, $database, $username, $password) {    
        $this->database_connection = new PDO(
            "{$engine}:dbname={$database};host={$host}", 
            $username, 
            $password, 
    ...
    private function prepareExecuteQuery($sql_statement, $placeholder_values = array(), $query_name = '') {    
        ...
        $query_object->execute(array_values($placeholder_values));
            ...
            if(is_array($where_clause)) {
                ...
                foreach($where_clause as $where_clause_column => $where_clause_value) {
                    if(is_null($where_clause_value)) {
                        throw new error("NULL value provided as a where clause placeholder value for column {$where_clause_column} in " . __METHOD__);
                    }
                    ...
                    if($where_column_row == 0) {
                        $query_where_clause .= "\nWHERE\t{$where_clause_column} {$column_equivalency} ?";
                    }
                    else {
                        $query_where_clause .= "\nAND\t{$where_clause_column} {$column_equivalency} ?";
                    }
    ...
    private function generateQuery($query_type, $table_name, $fields, $where_clause, $case_insensitive_columns = array(), $query_name = '') {                
        ...
        if(!empty($query_name) && isset($this->cached_queries[$cached_queries_name][$query_name])) {        
          ...
          $query_object = $this->prepareExecuteQuery($query, $placeholder_values, $query_name);
     ...
     * @return void
     */
    public function query($sql_statement, $placeholder_values = array()) {
        assert('is_array($placeholder_values)');
    
        $this->prepareExecuteQuery($sql_statement, $placeholder_values);
    }
}
